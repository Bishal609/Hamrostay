...
const verifyPayment = async (sessionId, userId) => {
  try {
    ...
    if (isPaid && payment.status !== "COMPLETED") {
      ...
      updates.stripePaymentId = session.payment_intent || payment.stripePaymentId;
    ...
    if (Object.keys(updates).length > 0) {
      await prisma.$transaction([
        prisma.payment.update({ where: { id: payment.id }, data: updates }),
        ...(isPaid && payment.booking.status !== "CONFIRMED"
