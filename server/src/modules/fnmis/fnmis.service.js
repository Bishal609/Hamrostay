...
const REVENUE_BOOKING_STATUSES = ["PENDING", "CONFIRMED", "CHECKED_IN", "CHECKED_OUT"];

const revenueBookingWhere = (dateField, dateFilter = {}) => ({
...
const getRevenue = async (period = "month") => {
  ...
  const [revenueBookings, bookings, totalRooms] = await Promise.all([
    prisma.booking.findMany({
      where: revenueBookingWhere("createdAt", { gte: startDate }),
  ...
  const totalRevenue = revenueBookings.reduce((sum, b) => sum + b.finalAmount, 0);

  // Group revenue by date
  const revenueByDate = {};
  revenueBookings.forEach((b) => {
    const date = b.createdAt.toISOString().split("T")[0];
    revenueByDate[date] = (revenueByDate[date] || 0) + b.finalAmount;
  });
...
const getDashboardKPIs = async () => {
  ...
  const [
    ...
    prisma.booking.aggregate({ where: revenueBookingWhere("createdAt", { gte: monthStart }), _sum: { finalAmount: true } }),
    prisma.booking.aggregate({ where: revenueBookingWhere("createdAt", { gte: lastMonthStart, lt: monthStart }), _sum: { finalAmount: true } }),
  ...
  const lastRevenue = lastMonthRevenue._sum.finalAmount || 0;
...
const getRevenueByRoomType = async () => {
  const bookings = await prisma.booking.findMany({
    ...
    select: { finalAmount: true, room: { select: { type: true } } },
  });

  const byType = {};
  bookings.forEach((b) => {
    const type = b.room?.type || "UNKNOWN";
    byType[type] = (byType[type] || 0) + b.finalAmount;
  });
