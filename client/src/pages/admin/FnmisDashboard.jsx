...
export default function FnmisDashboard() {
  ...
  const { data: kpis } = useQuery({ queryKey: ["fnmis-kpis"], queryFn: () => fnmisApi.getKPIs().then(r=>r.data.data) });
  const { data: revenue, isLoading: revLoading } = useQuery({ queryKey: ["fnmis-revenue", period], queryFn: () => fnmisApi.getRevenue(period).then(r=>r.data.data) });
  const { data: occupancy } = useQuery({ queryKey: ["fnmis-occ", period], queryFn: () => fnmisApi.getOccupancy(period).then(r=>r.data.data) });
  const { data: expenses } = useQuery({ queryKey: ["fnmis-expenses"], queryFn: () => fnmisApi.getExpenses({ limit: 20 }).then(r=>r.data.data) });
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Revenue",   value: `रू${(kpis?.revenue?.current||0).toFixed(0)}`,   change: kpis?.revenue?.growth, icon: DollarSign, color: "text-gold-400",    bg: "bg-gold-500/10" },
