...
import { ROOM_TYPES, ROOM_STATUSES, STATUS_COLORS } from "../../utils/constants";
import Loader from "../../components/common/Loader";
import toast from "react-hot-toast";

const EMPTY_ROOM = { roomNumber:"", name:"", type:"STANDARD", status:"AVAILABLE", floor:1, capacity:2, pricePerNight:100, description:"", amenities:[], images:[], size:300, bedType:"King", viewType:"", isFeatured:false, discount:0 };
...
export default function ManageRooms() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-rooms", { search, type, status, page }],
    queryFn: () => roomApi.getRooms({ search, type, status: status === "ALL" ? undefined : status, page, limit: 12, showAll: true }).then(r => r.data.data),
  });
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select value={form.status || "AVAILABLE"} onChange={e=>setForm({...form,status:e.target.value})} className="input text-sm">
                      {ROOM_STATUSES.map(s=><option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                </div>
